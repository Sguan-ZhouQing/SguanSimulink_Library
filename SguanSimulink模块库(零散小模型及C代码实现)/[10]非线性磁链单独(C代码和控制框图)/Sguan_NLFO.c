...
void Init(void){
    ...
    NLFO.Ld = 0.00056;
    NLFO.Lq = 0.00056;
    NLFO.Gain = 15000000.0;
    NLFO_Init(&NLFO);
}

void Loop(float Ia,float Ib,float Ua,float Ub,float *Sin,float *Cos){
    NLFO.go.Input_Ialpha = Ia;
    NLFO.go.Input_Ibeta = Ib;
    NLFO.go.Input_Ualpha = Ua;
    NLFO.go.Input_Ubeta = Ub;
    NLFO_Loop(&NLFO);
    *Cos = NLFO.go.Output_Cosine;
    *Sin = NLFO.go.Output_Sine;
}
